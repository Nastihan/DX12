...
inline void GetAssetsPath(_Out_writes_(pathSize) WCHAR* path, UINT pathSize)
{
    ...
    WCHAR Path[MAX_PATH]; // Assuming path is defined as WCHAR array
    DWORD PathSize = sizeof(Path);

    // Convert the WCHAR path to LPSTR (ANSI)
    CHAR ansiPath[MAX_PATH]; // Assuming you have a buffer to store the ANSI path
    WideCharToMultiByte(CP_ACP, 0, Path, -1, ansiPath, sizeof(ansiPath), NULL, NULL);

    DWORD size = GetModuleFileName(nullptr, ansiPath, sizeof(ansiPath));
